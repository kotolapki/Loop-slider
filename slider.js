(function() {
  ...
  const backSlideBtn = document.querySelector('.slider__controls-back');
  const forwardSlideBtn = document.querySelector('.slider__controls-forward');
  ...
  const LAST_SLIDE_DELAY = 510;
  const SHIFT_STEP = 12;
  const ACTIVE_DOT_CLASSNAME = 'slider__dots-item--active';
  ...
  function changeActiveDot(index) {
    for (const dot of dots) {
      if (dot.classList.contains(ACTIVE_DOT_CLASSNAME)) {
        dot.classList.remove(ACTIVE_DOT_CLASSNAME);
      }
    }
  
    if (index) {
      dots[index].classList.add(ACTIVE_DOT_CLASSNAME);
    } else {
      dots[currentSlideIndex].classList.add(ACTIVE_DOT_CLASSNAME);
    }
  }
}())